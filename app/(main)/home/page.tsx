...
const QuickAction = ({ icon: Icon, label, description, onClick, colorClass = "bg-green-50 text-green-600" }: any) => (
  <button 
    onClick={onClick}
  ...
  >
    ...
    <div>
      <span className="block font-semibold text-gray-800 group-hover:text-green-700 transition-colors">{label}</span>
...
const TransactionItem = ({ transaction, onClick }: { transaction: Transaction; onClick: () => void }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`text-right font-bold text-sm ${isDeposit ? "text-green-600" : "text-gray-800"}`}>
...
export default function HomePage() {
  ...
  const { data: walletData, isLoading: walletLoading } = useSWR<ApiResponse<Wallet>>("/wallet/me", fetcher);
  const { data: transactionsData, isLoading: transactionsLoading } = useSWR<ApiResponse<Transaction[]>>("/transactions/me?limit=5", fetcher);
                ...
                <div className="space-y-1">
                  {transactionsLoading ? (
                    ...
                    <div className="text-center py-12">
                      <Icons.Empty />
                      <p className="text-gray-500 text-sm mb-4">Belum ada aktivitas bulan ini</p>
                      <button 
                        onClick={() => router.push("/setor-online")}
