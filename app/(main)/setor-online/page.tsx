...
const RequestCard = ({ request, onClick }: { request: DepositRequest; onClick: () => void }) => {
  const config = STATUS_CONFIG[request.status as DepositRequestStatus] || STATUS_CONFIG[DepositRequestStatus.PENDING];
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-3">
        ...
        {/* Status Badge */}
        <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${config.color}`}>
          {config.icon}
      ...
      <div className="flex items-center gap-4 mt-4 pt-4 border-t border-gray-50">
         ...
         {/* Photo Indicator (Only if photo exists) */}
         {request.photo && (
      ...
      <div className="min-h-screen bg-gray-50/30 pb-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Setor Sampah Online</h1>
              <p className="text-sm text-gray-500 mt-1">Kelola permintaan penjemputan sampahmu disini.</p>
            </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
            {[
              ...
              { value: DepositRequestStatus.APPROVED, label: "Dijemput" },
              ...
              { value: DepositRequestStatus.REJECTED, label: "Ditolak" },
                ...
                return (
                    <button
                        key={tab.value}
                        onClick={() => setFilter(tab.value as DepositRequestStatus | "ALL")}
          ...
          <div className="space-y-4">
            {isLoading ? (
              ...
              <div className="bg-white rounded-3xl p-10 text-center border border-dashed border-gray-200">
                ...
                <h3 className="text-gray-900 font-bold text-lg">Belum ada request</h3>
              ...
              filteredRequests.map((request) => (
                <RequestCard
                  ...
                  request={request}
                  onClick={() => router.push(`/setor-online/${request.request_id}`)}
                />
              ))
            )}
          </div>
